...
/* ScrollReelTestimonials — counter-rotating scroll reel + per-character text rise.
 * Adapted from scroll-reel-testimonials for this project's design tokens. */
...
const QUOTE_CLASSES =
  "m-0 font-display text-xl font-medium leading-[1.3] tracking-tight text-ink sm:text-2xl";
...
function Cell() {
  return (
    <div
      aria-hidden="true"
      className="shrink-0 rounded-xl border border-line bg-gradient-to-b from-line to-surface blur-[1px] shadow-[0_1px_2px_rgba(0,0,0,0.05),inset_0_2px_0_rgba(255,255,255,1)]"
...
function Featured({ src, alt }: { src: string; alt?: string }) {
  return (
    <div
      className="relative shrink-0 overflow-hidden rounded-xl bg-line"
    ...
    >
      ...
      <div
        ...
        style={{
          background:
            "linear-gradient(220.99deg, rgba(82,231,206,0) 32%, rgb(82,231,206) 41%, rgb(150,240,224) 47%, rgba(130,189,237,0.5) 54%, rgba(130,189,237,0) 65%)",
        }}
      />
    </div>
  );
...
export function ScrollReelTestimonials({
  ...
  return (
    <div
      ...
      onKeyDown={onKeyDown}
      className={cn(
        "relative flex w-full max-w-[1240px] flex-col items-stretch gap-4 overflow-hidden rounded-2xl bg-transparent outline-none focus-visible:ring-2 focus-visible:ring-accent md:min-h-[420px] md:flex-row md:gap-8",
        className
      )}
    >
      <div
        aria-hidden="true"
        className="relative h-72 w-full shrink-0 self-stretch overflow-hidden md:h-auto md:w-[560px]"
        style={{
          WebkitMaskImage:
      ...
      <div className="flex min-w-0 flex-1 flex-col justify-between self-stretch px-5 py-7 md:py-10">
        <div className="flex flex-col gap-[9px]">
          <svg className="block h-12 w-12 text-accent/50" viewBox="0 0 24 24" fill="currentColor" aria-hidden="true">
